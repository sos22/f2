...
public:  filesystemservice(const constoken &token,
    ...
    : rpcservice2(token, interfacetype::filesystem),
      fs(filesystemimpl::filesystem::build(_cp)) {}
...
filesystemagent(clientio io,
    ...
    if (r.isfailure()) {
        cp.success()->destroy();
        return r.failure(); }
    else return nnp<rpcservice2>(*r.success()); }
